    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                PlaceList allsuccessors =
                        model.allSuccessors(sq.x, sq.y, sq.direction());
                for (Place successsor : allsuccessors) {
                    Sq thissquare = model.get(successsor);
                    if (sq.connectable(thissquare)) {
                        nFound += 1;
                        found = thissquare;
                        if (sq.sequenceNum() != 0
                                && thissquare.sequenceNum() != 0) {
                            nFound = 1;
                            found = thissquare;
                        }
                    }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                PlaceList allpredecessors =  sq.predecessors();
                for (Place predecessor : allpredecessors) {
                    Sq thissquare = model.get(predecessor);
                    if (sq.connectable(thissquare)) {
                        nFound += 1;
                        found = thissquare;
                        if (sq.sequenceNum() != 0
