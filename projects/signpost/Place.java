...
class Place {
    ...
    static PlaceList[][][] successorCells(int width, int height) {
        /** array contains all 9 directions, all directions and each seperate direction */
        PlaceList[][][] M =
                new PlaceList[width][height][9];
        int lim = Math.max(width, height);
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                PlaceList alldirections = new PlaceList();
                for (int i = 1; i <= 8; i++) {
                    PlaceList particulardirection = new PlaceList();
                    int directionx = dx(i);
                    int directiony = dy(i);
                    for (int j = 1; j < lim; j++) {
                        ...
                        if (newx < width && newy < height
                                && newx >= 0 && newy >= 0) {
                            particulardirection.add(pl(newx, newy));
                            alldirections.add(pl(newx, newy));
                        }
                    }
                    M[x][y][i] = particulardirection;
                }
                M[x][y][0] = alldirections;
            }
        }
        return M;
    }
